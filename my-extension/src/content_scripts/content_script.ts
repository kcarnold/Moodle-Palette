...
(function() {
    ...
    function getCourseId() {
        ...
        const courseLink = document.querySelector('header a[href*="/course/view.php"]') as HTMLAnchorElement;
        return new URL(courseLink.href).searchParams.get('id');
    }
    ...
    if (window.location.pathname === '/course/view.php') {
        ...
        activityDirectory = [...document.querySelectorAll('li.section')].map(section => {
            ...
            let activities = [...section.querySelectorAll('ul.section > li.activity .activityinstance')].map(activityInstance => {
                ...
                let activity = {title, url, type: null, id: null``};
    ...
    let ninjaData = [];
    let courseSections = [
        {title: "Course Home", handler: () => {window.location.href = `/course/view.php?id=${courseId}&perpage=5000`; }},
        {
            id: "Participants",
            title: "Course Participants",
            handler: () => {window.location.href = `/user/index.php?id=${courseId}`; }
        },
        {title: "Gradebook", handler: () => {window.location.href = `/grade/report/index.php?id=${courseId}`; }}
    ];
    (() => {
        let x;
